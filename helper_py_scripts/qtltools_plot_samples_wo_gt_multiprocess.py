...
def process_file(f, op_png_dir):
    ...
    don = os.path.basename(f).replace(".bamstat.txt", "").split('_')[1]
    ...
    # ------ Plot top 2 sample with highest hets -----
    ...
    # ------ Plot top 2 sample with highest homs -----
    f1_sorted = f1.sort_values(
...
def main():
    output_dir = "/sc/arion/projects/CommonMind/pnm/genesis_U24/A16_McCarroll_SCZ/"
    vir_out_dir = "/sc/arion/projects/psychAD/pnm/A16_McCarroll_SCZ/demultiplex/vireoSNP_w_gt/"
    qtltools_op_dir = "/sc/arion/projects/CommonMind/pnm/genesis_U24/A16_McCarroll_SCZ/qtltools_mbv_w_gt/"
    proj_name = 'A16_McCarroll_SCZ'
    gt_round = 'wo_gt' # Typically, wo_gt, w_gt, w_gt2/w_gt_run2, etc.
    max_workers = 16   # Adjust to your CPU cores

    all_files_qtltools = glob2.glob(os.path.join(qtltools_op_dir, "*.txt"))
    op_png_dir = '/'.join(qtltools_op_dir.split('/')[:-1])+'_pngs' \
                if qtltools_op_dir[-1] == '/' \
                else qtltools_op_dir + '_pngs'

    all_files_demux=glob2.glob(os.path.join(vir_out_dir, "**/summary.tsv"))

    os.makedirs(op_png_dir, exist_ok=True)
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(process_file, f, op_png_dir)
            for f in sorted(all_files_qtltools)
        ]
    ...
    for f in sorted(all_files_qtltools):
        ...
        if len(uniq_id.keys()) == 0 or os.path.join(vir_out_dir, f"{samp}/summary.tsv") not in uniq_id:
            summ_f = os.path.join(vir_out_dir, f"{samp}/summary.tsv")
            temp_f = pd.read_csv(summ_f, sep='\t')
            uniq_id[summ_f] = temp_f
        else:
            temp_f= uniq_id[os.path.join(vir_out_dir, f"{samp}/summary.tsv")]
        try:
    ...
    a_df.to_csv(os.path.join(output_dir, f"{proj_name}_{gt_round}_res.tsv"), sep='\t', index=False)
    ...
    b_df.to_csv(os.path.join(output_dir, f"{proj_name}_{gt_round}_res2.tsv"), sep='\t', index=False)

    
    c = []
    for f in sorted(all_files_demux):
        samp = f.split('/')[-2]
    ...
    c_df.to_csv(os.path.join(output_dir, f"{proj_name}_vir_demux_stats_{gt_round}.tsv"), sep='\t', index=False)
